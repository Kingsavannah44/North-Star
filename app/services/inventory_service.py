...
def check_availability(db: Session, product_id: int, size: Optional[str] = None) -> dict:
    """
    Check stock availability for a product, optionally filtered by size.

    Returns a dict with product_id, product_name, size, available, quantity.
    Raises ProductNotFoundError or SizeNotFoundError as appropriate.
    """
    ...
    if size:
        # Look for this exact size (case-insensitive)
        inventory_row = (
            db.query(Inventory)
            .filter(
    ...
    # No size filter — aggregate total stock across all sizes
